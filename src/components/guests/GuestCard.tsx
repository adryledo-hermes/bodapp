...
export default function GuestCard({
  ...
  guest: GuestCardData;
  locale: Locale;
  onEdit?: (guest: GuestCardData) => void;
}) {
  ...
  return (
    <div className="group relative block w-full text-left [perspective:1200px]">
      <button
        type="button"
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center overflow-hidden rounded-2xl border border-slate-200 bg-white p-4 shadow-sm [backface-visibility:hidden]">
            {guest.photoUrl ? (
              ...
              <img
                src={guest.photoUrl}
                alt={guest.fullName}
                className="mb-2 h-16 w-16 rounded-full object-cover ring-2 ring-indigo-100"
              />
            ) : (
