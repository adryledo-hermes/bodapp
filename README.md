...
- **Invitations**: per-guest/group invitations created manually (**already-invited guests are disabled** in the picker); each invitation is created **from the Invitation Template** (frame + image + base copy) **adding the guests' names**; each can be further personalised (frame, image, text overrides) with a **live preview** and its **QR underneath** (the old QR panel is gone; `/qr` redirects to `/invitaciones`). The template editor (`/invitacion`) shares the same fields: frame (real floral images), image, names, message, **date/time pickers**, venue, dress code, colours.
...
## Roadmap
- ✅ **v1.2.1** — Mobile nav drawer; guest context dropdown + allergy multiselect + music split (genres + favourite song); seating fixes (same-table re-seat, table size by capacity, PATCH-only-changed-fields so position/capacity never reset); profile photo vs wedding gallery separation; invitation panel upgrades (disable already-invited, rich per-invitation editor with frame/image/text, QR inline under each invitation, QR panel removed).
